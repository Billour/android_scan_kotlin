package com.repongroup.repon_android
...
import android.view.*
import kotlinx.android.synthetic.main.repon_list.*
import android.widget.*
import com.repongroup.repon_android.Repon_android.Companion.appname
...
import com.repongroup.repon_android.prividers.AppLogDb.Companion.action_action
import com.repongroup.repon_android.prividers.AppLogDb.Companion.action_date
import com.repongroup.repon_android.prividers.AppLogDb.Companion.action_view
import com.repongroup.repon_android.prividers.AppLogDb.Companion.delete
import com.repongroup.repon_android.prividers.AppLogDb.Companion.insert
import com.repongroup.repon_android.prividers.AppLogDb.Companion.insert_date
import com.repongroup.repon_android.prividers.AppLogDb.Companion.rawquery
import com.repongroup.repon_android.prividers.AppLogDb.Companion.status
import java.util.concurrent.Executor
import java.util.concurrent.Executors
import kotlin.collections.ArrayList
import kotlin.collections.HashMap

class Repon_list: AppCompatActivity() {

    private val context: Context = this
    private var today: String = "" //記錄今天的日期
    private lateinit var reponview: View //宣告自定義dialog
    private lateinit var reponAlertDialog: ReponAlertDialog
    private lateinit var reponAlertDialog2: ReponAlertDialog
    private lateinit var m_repon_list_login_E_account: EditText
    private lateinit var m_repon_list_login_E_password: EditText
    ...
    private val REPONDATACOLUMN: Array<String> = arrayOf("id","so_date","so_nbr","so_nbr_step")
    ...
    private val packing_scan = arrayOf("台北廠","雲科廠","南俊商貿") //打包區資料表中文清單
    private val packing_scan_id = arrayOf("packing_scan_NJ","packing_scan_RP","packing_scan_SZ") //打包區db資料表清單
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.repon_list)
    ...
    private fun setupViewComponent() {
        ...
        repon_list_T_gettoday.text = today
        repon_list_T_gettoday.setTextColor(ContextCompat.getColor(context, R.color.blue))

        layoutstart() //layout初始化設定
        repondialog() //初始化工程模式登入視窗
        ...
        val msg = resources.getString(R.string.repon_list_t_loginmsg) + newmsg
        val redstart = resources.getString(R.string.repon_list_t_loginmsg).length
        ...
        repon_list_T_loginmsg.setText(msgcolor, TextView.BufferType.SPANNABLE)
        //------------------------------------------------------------------------------------------

        repon_list_T_loginmsg.setOnClickListener(showlogin()) //login提示訊息監聽
    }

    //layout初始化設定
    private fun layoutstart() {
        repon_list_R_logo.visibility = View.VISIBLE //顯示Logo頁面
        repon_list_R_rel.visibility = View.GONE //殺掉工程模式頁面
        repon_list_R_query.visibility = View.GONE //殺掉工程模式-資料表查詢頁面
    }

    //初始化工程模式登入視窗
    private fun repondialog() {
        reponview = LayoutInflater.from(context).inflate(R.layout.repon_list_login,null)

        //設定選單選擇視窗
        reponAlertDialog = ReponAlertDialog(context)
        reponAlertDialog.setView(reponview,0,0,0,
                0) //設定自定義layout
        m_repon_list_login_E_account = reponview.findViewById(R.id.repon_list_login_E_account) //帳號
        m_repon_list_login_E_password = reponview.findViewById(R.id.repon_list_login_E_password) //密碼
        reponAlertDialog.show() //務必先show出來才能設定參數

        //自定義Dialog視窗參數
        val params: WindowManager.LayoutParams = reponAlertDialog.window.attributes //取得dialog參數對象
        ...
        reponAlertDialog.window.attributes = params //dialog參數綁定

        reponlog("Repon_list login_layout start.",1) //寫log
    }

    //寫log
    private fun reponlog(getaction: String, getstatus: Int){
    ...
    private fun cleardata() {
        ...
        singleThreadExecutor.execute(Runnable {
            val sqlite: String = "SELECT insert_date FROM repon_user_action ORDER BY id DESC"
    ...
    private fun showlogin(): View.OnClickListener? = View.OnClickListener{
        repondialog() //開啟工程模式登入視窗
        reponlog(classname + " login_layout start.",1) //寫log
    }

    //reponAlertDialog上的login按鈕
    fun login(view: View) {
        val account = m_repon_list_login_E_account.text.toString().trim { it <= ' ' }
        val password = m_repon_list_login_E_password.text.toString().trim { it <= ' ' }
//        Log.d(TAG,"account=$account ,password=$password")

        if (account != "" && password != "" && account == "sa" && password == "password"){ //驗證帳密,repon_acget(account) && repon_pwget(password):要再寫
            Toast.makeText(context,"登入成功", Toast.LENGTH_SHORT).show()
            reponlog("Accout:******** login success.",1) //寫log
            actionshow() //顯示使用者活動列表
            repon_list_R_logo.visibility = View.GONE //殺掉Logo頁面
            repon_list_R_rel.visibility = View.VISIBLE //顯示工程模式頁面
            ...
            m_repon_list_login_E_password.setText("") //清空輸入的密碼
            reponlog("Accout:******** login failure.",0) //寫log
        }
    ...
    private fun actionshow() {
        ...
        singleThreadExecutor.execute {
            var error = 0
            var sql = "SELECT * FROM repon_user_action WHERE 1=1 AND app_name = '$appname'" +
            ...
            try {
                ...
                sql = "SELECT * FROM repon_user_action WHERE 1=1 AND app_name = '$appname' ORDER BY " +
                ...
                if (data_show > 0){ //有資料,設定筆數
                    ...
                    for (i in 0..(recSet.size - 1)){
                        var item: HashMap<String, Any> = HashMap()
            ...
            runOnUiThread {
                ...
                val adapter: SimpleAdapter = SimpleAdapter(
                        context,
                        mList,
                        R.layout.repon_list_item,
                        arrayOf("textview"),
                        intArrayOf(R.id.repon_list_item_T_data)
                )
                //--------------------
                adapter.notifyDataSetChanged() //通知UI更新數據
                repon_list_L_menu.adapter = adapter
                repon_list_L_menu.isEnabled = true
    ...
    private fun textmsg() {
        ...
        repon_list_T_msg.setText(msgcolor, TextView.BufferType.SPANNABLE)
        Toast.makeText(context, msg2, Toast.LENGTH_SHORT).show()
    }

    //reponAlertDialog上的yes按鈕
    fun yesbtn(view: View) {
        reponlog("User click yesbtn to finish this.",1) //寫log
        reponAlertDialog.dismiss() //關閉dialog視窗
        Toast.makeText(context,"已離開工程模式", Toast.LENGTH_SHORT).show()
        this.finish()
    }

    //reponAlertDialog上的calcel按鈕
    ...
    fun I_setting(view: View) {
        reponview = LayoutInflater.from(context).inflate(R.layout.repon_list_menu,null) //自定義Layout:dialog
        reponAlertDialog = ReponAlertDialog(context)
        reponAlertDialog.setView(reponview,0,0,0,
                0)
    ...
    fun sqlbtn(view: View) {
        reponview = LayoutInflater.from(context).inflate(R.layout.repon_list_menu_sql,null) //自定義Layout:dialog
        reponAlertDialog2 = ReponAlertDialog(context)
        reponAlertDialog2.setView(reponview,0,0,0,
                0)
        m_sql_E_write = reponview.findViewById(R.id.sql_E_write) //SQLite_where以後語法輸入框
        val begin = "SELECT * FROM repon_user_action ".length
...
//        Log.d(TAG,"user_write=$user_write")
        ...
        reponAlertDialog2.show() //務必先show出來才能設定參數

        //自定義Dialog視窗參數
        val params = reponAlertDialog2.window.attributes //取得dialog參數對象
        params.width = WindowManager.LayoutParams.WRAP_CONTENT //設置dialog寬度包裹內容
        params.height = WindowManager.LayoutParams.WRAP_CONTENT //設置dialog高度包裹內容
        params.gravity = Gravity.CENTER //設置dialog重心
        reponAlertDialog2.window.attributes = params //dialog參數綁定
    ...
    fun querybtn(view: View) {
        ...
        when(str.substring(0,1)){
            "w","W","o","O" -> {
                user_write = "SELECT * FROM repon_user_action $str"
        ...
        repon_list_R_rel.visibility = View.VISIBLE //顯示工程模式頁面
        repon_list_R_query.visibility = View.GONE //殺掉工程模式-資料表查詢頁面
        reponAlertDialog2.dismiss() //關閉dialog
    }

    //SQL查詢視窗：欄位說明
    fun supportbtn(view: View) {
        reponview = LayoutInflater.from(context).inflate(R.layout.repon_list_sql_support,null) //自定義Layout:dialog
        reponAlertDialog = ReponAlertDialog(context)
        reponAlertDialog.setView(reponview,0,0,0,
                0) //設定自定義layout
        val m_support_T_detail: TextView = reponview.findViewById(R.id.support_T_detail) //LOG欄位說明視窗
        ...
        //自定義Dialog視窗參數
        val params = reponAlertDialog.window.attributes //取得dialog參數對象
        params.width = WindowManager.LayoutParams.WRAP_CONTENT //設置dialog寬度包裹內容
        params.height = WindowManager.LayoutParams.WRAP_CONTENT //設置dialog高度包裹內容
        params.gravity = Gravity.CENTER //設置dialog重心
        reponAlertDialog.window.attributes = params //dialog參數綁定
    ...
    fun tablebtn(view: View) {
        reponview = LayoutInflater.from(context).inflate(R.layout.repon_list_menu_table,null) //自定義Layout:dialog
        reponAlertDialog2 = ReponAlertDialog(context)
        reponAlertDialog2.setView(reponview,0,0,0,
                0) //設定自定義layout
        m_table_S_db = reponview.findViewById(R.id.table_S_db) //資料庫選單
        m_table_S_table = reponview.findViewById(R.id.table_S_table) //資料表選單
        m_table_E_sql_where = reponview.findViewById(R.id.table_E_sql_where) //SQLite_where語法
        m_table_E_sql_order = reponview.findViewById(R.id.table_E_sql_order) //SQLite_order語法
        db_spinner() //資料庫項目生成
    ...
    fun tablecheckbtn(view: View){
        ...
        singleThreadExecutor.execute {
            ...
            val authority = "com.repongroup.${table_id[table_num]}.providers.${cp[table_num]}" //內容提供者路徑
            var quri: Uri? = null
            ...
            runOnUiThread {
                ...
                //----------------------------------
                adapter.notifyDataSetChanged() //通知UI更新數據
                repon_list_L_menu2.adapter = adapter
                repon_list_L_menu2.isEnabled = true
                ...
                if (error == 1){
                    ...
                    repon_list_E_sql_where.setText(sql_where) //顯示使用者輸入的SQL_where語法
                    repon_list_E_sql_order.setText(sql_order) //顯示使用者輸入的SQL_order語法
                    repon_list_R_rel.visibility = View.GONE //殺掉工程模式頁面
                    repon_list_R_query.visibility = View.VISIBLE //顯示工程模式-資料表查詢頁面
                    textmsg2() //設定訊息框
                    reponAlertDialog2.dismiss() //關閉dialog
                }
    ...
    private fun textmsg2() {
        ...
        repon_list_T_msg2.setText(msgcolor, TextView.BufferType.SPANNABLE)
        Toast.makeText(this, msg2, Toast.LENGTH_SHORT).show()
    }

    //工程模式-資料表查詢裡的查詢按鈕
    fun requerybtn(view: View){
        sql_where = repon_list_E_sql_where.text.toString()
        sql_order = repon_list_E_sql_order.text.toString()
    ...
    fun logdelbtn(view: View) {
        reponview = LayoutInflater.from(this).inflate(R.layout.repon_list_delcheck, null) //自定義Layout:dialog
        reponAlertDialog2 = ReponAlertDialog(context)
        reponAlertDialog2.setView(reponview,0,0,0,
                0) //設定自定義layout
        reponAlertDialog2.show() //務必先show出來才能設定參數
        ...
        reponAlertDialog.dismiss() //關閉上一個dialog
    }
    ...
    fun delyesbtn(view: View){
        ...
        reponAlertDialog2.dismiss() //執行後關閉
    }
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent?): Boolean {
        ...
        if (keyCode == KeyEvent.KEYCODE_BACK){
            ...
            if (System.currentTimeMillis() - exitTime > 2000){
                ...
                if (repon_list_R_query.visibility == View.VISIBLE) //工程模式-資料表查詢頁面開啟
                    Toast.makeText(this, msg2, Toast.LENGTH_SHORT).show()
                else
                    Toast.makeText(this, msg, Toast.LENGTH_SHORT).show()
            }else {

                if (repon_list_R_rel.visibility == View.VISIBLE){
                    reponview = LayoutInflater.from(context).inflate(R.layout.repon_list_quit,null) //自定義Layout:dialog

                    //設定選單選擇視窗
                    reponAlertDialog = ReponAlertDialog(context)
                    reponAlertDialog.setView(reponview,0,0,0,
                            0) //設定自定義layout
                    reponAlertDialog.setCancelable(false) //dialog顯示時不能用其他方式關掉
                    reponAlertDialog.show() //務必先show出來才能設定參數
                ...
                }else if (repon_list_R_query.visibility == View.VISIBLE){ //工程模式-資料表查詢頁面開啟
                    repon_list_R_query.visibility = View.GONE //殺掉資料表查詢頁面
                    repon_list_R_rel.visibility = View.VISIBLE //顯示工程模式

                }else {
                    reponlog("User click mobile_backbtn second to close this.",1) //寫log
