package com.repongroup.repon_android.prividers
...
class AppLogDb(context: Context?, name: String?, factory: SQLiteDatabase.CursorFactory?, version: Int) :
    ...
    companion object { //同java中的static
        private val DB_NAME: String = "repon.db" //資料庫名稱
        private val VERSION: Int = 1 //資料庫版本，資料庫結構改變時要更改數字，通常+1
        private val DB_TABLE: String = "repon_user_action" //資料表名稱
        ...
        private var db_repon_list: SQLiteDatabase? = null //資料庫名稱
        ...
        fun getDatabase(context: Context): SQLiteDatabase? {
            ...
            if (db_repon_list == null || !db_repon_list!!.isOpen) {
                db_repon_list = AppLogDb(context, DB_NAME,null, VERSION).writableDatabase
            }

            return db_repon_list
        }
        ...
        fun rawquery(context: Context, sql: String, selectionArgs: Array<String>): Cursor? {
            ...
            try {
                 c = db_repon_list?.rawQuery(sql, selectionArgs)
            }catch (e: Exception){
                e.toString()
            }
        ...
        fun insert(context: Context): Long{
            ...
            val rowId: Long = db_repon_list?.insert(DB_TABLE,null, newRow) ?: 0
            db_repon_list!!.close() //寫入完成關閉
...
//刪除資料：殺log
        fun delete(context: Context, where: String): Int{
            ...
            val c: Cursor? = db_repon_list?.rawQuery(sql,null)

            if (c?.count ?: 0 != 0){
                c!!.close()
                val rowsAffected: Int = db_repon_list?.delete(DB_TABLE, where,null) ?: 0
                db_repon_list!!.close()
